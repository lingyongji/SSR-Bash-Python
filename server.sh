...
while :; do echo
    read -p "请选择： " serverc
    [ -z "$serverc" ] && ssr && break
    if [[ $serverc -ge 1 -a $serverc -le 10 ]]; then
        echo "输入错误! 请输入正确的数字!"
    else
        break
    fi
...
if [[ $serverc == 10 ]];then
    echo "1.修改为详细模式(输出详细连接日志+错误日志)"
    echo "2.修改为简单模式(仅输出错误日志)"
    while :; do echo
        read -p "请选择： " logtype
        if [[ ! $logtype =~ ^[1-2]$ ]]; then
            echo "输入错误! 请输入正确的数字!"
    ...
    if [[ $logtype == 1 ]];then
        sed -i 's/"connect_verbose_info": 0/"connect_verbose_info": 1/g' /usr/local/shadowsocksr/user-config.json
    else
        sed -i 's/"connect_verbose_info": 1/"connect_verbose_info": 0/g' /usr/local/shadowsocksr/user-config.json
    fi
    echo "日志模式已修改"
    bash /usr/local/SSR-Bash-Python/server.sh
fi